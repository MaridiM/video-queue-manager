...
### Фаза 1: Настройка Backend

- [ ] Установить npm пакет `dropbox`
- [ ] Создать `services/dropboxService.js`
- [ ] Обновить структуру `settings.json` (добавить секцию `dropbox`)
- [ ] Обновить функцию `loadSettings()`
- [ ] Обновить функцию `saveSettings()`
- [ ] Добавить эндпоинты API настроек Dropbox:
  - [ ] `GET /api/settings/dropbox`
  - [ ] `PUT /api/settings/dropbox`
  - [ ] `POST /api/settings/dropbox/test`
- [ ] Заменить операции чтения CSV на Dropbox API
- [ ] Заменить сохранение транскрипций на Dropbox API
- [ ] Добавить обработку ошибок и логику повторов
- [ ] Добавить логирование операций Dropbox

### Фаза 2: Настройка Frontend

- [ ] Обновить `lib/api.ts` (добавить клиент Dropbox API)
- [ ] Обновить `pages/Settings.tsx` (добавить секцию Dropbox)
- [ ] Создать `components/DropboxSettingsCard.tsx`
- [ ] Добавить поле ввода токена (тип password)
- [ ] Добавить кнопку "Тест подключения"
- [ ] Добавить индикатор статуса подключения
- [ ] Добавить сообщения об ошибках для неудачных операций
- [ ] Обновить макет страницы настроек (разделить AI и Dropbox)

### Фаза 3: Миграция

- [ ] Реализовать двойной режим (Dropbox + локальный откат)
- [ ] Протестировать с существующими данными
- [ ] Мигрировать все операции с файлами
- [ ] Удалить код локальной файловой системы
